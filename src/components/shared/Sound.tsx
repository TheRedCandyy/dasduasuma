import { useEffect } from 'react'
import { useTranslation } from 'react-i18next'
import { useSoundStore } from '@/store/useSoundStore'

// This hook is kept for backward compatibility
export const useSoundEnabled = () => {
  const { soundEnabled, setSoundEnabled } = useSoundStore()
  return { soundEnabled, setSoundEnabled }
}

interface SoundToggleProps {
  minimal?: boolean
}

const SoundToggle = ({ minimal = false }: SoundToggleProps) => {
  const { t } = useTranslation()
  const { soundEnabled, setSoundEnabled } = useSoundStore()

  useEffect(() => {
    // Initialize or control audio based on sound enabled state
    if (soundEnabled) {
      // Play background sound
      const audio = new Audio('/audio/dasduasuma.mp3')
      audio.loop = true
      audio.volume = 0.5
      audio.play().catch(error => {
        console.error('Error playing audio:', error)
        // setSoundEnabled(false) // Zustand persist will handle initial state
      })

      return () => {
        audio.pause()
        audio.currentTime = 0
      }
    }
  }, [soundEnabled]) // Removed setSoundEnabled from dependencies to avoid potential loops with cookie logic

  const handleToggleSound = () => {
    const newSoundState = !soundEnabled
    setSoundEnabled(newSoundState)
    // Cookies.set('userSoundPreference', String(newSoundState), { expires: 365 }) // Removed, Zustand persist handles this
  }

  return (
    <button
      onClick={handleToggleSound} // Use new handler
      className={`hover:cursor-pointer flex items-center w-full justify-center ${minimal ? 'space-x-1' : 'space-x-2'} text-slate-800 focus:outline-none`}
      aria-label={soundEnabled ? t('sound.disable') : t('sound.enable')}
    >
      <div className="relative">
        {/* Sound icon */}
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          fill="currentColor"
          className={`${minimal ? 'w-4 h-4' : 'w-5 h-5'} transition-opacity duration-300`}
        >
          <path d="M13.5 4.06c0-1.336-1.616-2.005-2.56-1.06l-4.5 4.5H4.508c-1.141 0-2.318.664-2.66 1.905A9.76 9.76 0 001.5 12c0 .898.121 1.768.35 2.595.341 1.24 1.518 1.905 2.659 1.905h1.93l4.5 4.5c.945.945 2.561.276 2.561-1.06V4.06zM18.584 5.106a.75.75 0 011.06 0c3.808 3.807 3.808 9.98 0 13.788a.75.75 0 11-1.06-1.06 8.25 8.25 0 000-11.668.75.75 0 010-1.06z" />
          <path d="M15.932 7.757a.75.75 0 011.061 0 6 6 0 010 8.486.75.75 0 01-1.06-1.061 4.5 4.5 0 000-6.364.75.75 0 010-1.06z" />
        </svg>

        {/* Mute overlay */}
        {!soundEnabled && (
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="currentColor"
            className={`absolute inset-0 ${minimal ? 'w-4 h-4' : 'w-5 h-5'} text-red-500`}
          >
            <path
              fillRule="evenodd"
              d="M2.47 2.47a.75.75 0 011.06 0l18 18a.75.75 0 11-1.06 1.06l-18-18a.75.75 0 010-1.06z"
              clipRule="evenodd"
            />
          </svg>
        )}
      </div>

      {!minimal && (
        <span className="text-base font-medium">
          {soundEnabled ? t('sound.disable') : t('sound.enable')}
        </span>
      )}
    </button>
  )
}

export default SoundToggle
